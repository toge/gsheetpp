...
/**
 * @file test_google_sheets_client.cpp
 * @brief Google Sheets クライアントの主要挙動を検証するテストです。
 */

namespace {

/**
 * @brief JWT 署名テスト用の固定秘密鍵を返します。
 * @return テスト専用 PEM 文字列です。
 */
...
/**
 * @brief RAII ガードが例外時でも refresh 状態を解除することを検証します。
 */
...
/**
 * @brief サービスアカウント JSON の解析とバリデーションを検証します。
 */
...
/**
 * @brief 各種レスポンス JSON の変換処理を検証します。
 */
...
/**
 * @brief JWT とリクエスト本文の組み立て処理を検証します。
 */
TEST_CASE("jwt assertion contains required claims", "[jwt]") {
  ...
  CHECK(decoded.get_payload_claim("scope").as_string() == "https://www.googleapis.com/auth/spreadsheets");
  ...
  CHECK(decoded.get_expires_at().time_since_epoch() == (issued_at + std::chrono::hours{1}).time_since_epoch());
}
...
TEST_CASE("token request body includes JWT bearer grant type", "[requests]") {
  ...
  CHECK(body.find("grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Ajwt-bearer") != std::string::npos);
...
TEST_CASE("network failures surface as network errors", "[client]") {
  ...
  auto client = gsheetpp::detail::make_google_sheets_client_for_test(config, [](gsheetpp::detail::HttpRequest const&) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> {
...
TEST_CASE("read_values_async falls back to generic http error for malformed API error payload", "[client]") {
  auto const config = gsheetpp::ServiceAccountConfig{
      .client_email = "svc@example.iam.gserviceaccount.com",
      .private_key  = make_test_private_key(),
      .token_uri    = "https://oauth2.googleapis.com/token",
...
/**
 * @brief 認証方式の切り替えと API key 挙動を検証します。
 */
TEST_CASE("set_authenticator rebinds the client type", "[client][auth]") {
  auto client = gsheetpp::GoogleSheetsClient{gsheetpp::ServiceAccountConfig{
      .client_email = "svc@example.iam.gserviceaccount.com",
      .private_key  = make_test_private_key(),
      .token_uri    = "https://oauth2.googleapis.com/token",
      .project_id   = "demo-project",
...
TEST_CASE("api key authenticate_async is a no-op success", "[client][api-key]") {
  auto client = gsheetpp::BasicGoogleSheetsClient{
      ...
      [](gsheetpp::detail::HttpRequest const&) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> { FAIL("api key authenticate_async should not hit the transport"); }};
...
TEST_CASE("api key authenticate_async stays a no-op success even when the key is empty", "[client][api-key]") {
...
TEST_CASE("api key reads append the key query parameter without authorization header", "[client][api-key]") {
  auto client = gsheetpp::BasicGoogleSheetsClient{gsheetpp::ApiKeyAuth{
                                                      .api_key = "demo-key",
                                                  },
                                                  [](gsheetpp::detail::HttpRequest const& request) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> {
                                                    CHECK(request.method == "GET");
                                                    CHECK(request.url == "https://sheets.googleapis.com/v4/spreadsheets/spreadsheet-id/values/Sheet1%21A1%3AB2?key=demo-key");
...
TEST_CASE("api key writes fail before issuing an HTTP request", "[client][api-key]") {
  ...
  auto       client           = gsheetpp::BasicGoogleSheetsClient{gsheetpp::ApiKeyAuth{
                                                  ...
                                                  [&](gsheetpp::detail::HttpRequest const&) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> {
...
TEST_CASE("api key reads reject an empty key before issuing an HTTP request", "[client][api-key]") {
  auto transport_called = false;
  auto client           = gsheetpp::BasicGoogleSheetsClient<gsheetpp::ApiKeyAuth>{gsheetpp::ApiKeyAuth{},
                                                                                  [&](gsheetpp::detail::HttpRequest const&) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> {
                                                                          transport_called = true;
                                                                          return gsheetpp::detail::HttpResponse{};
                                                                        }};
...
/**
 * @brief User OAuth2 の token 取得・更新・再試行挙動を検証します。
 */
TEST_CASE("user oauth authenticate_async exchanges an authorization code and persists refresh token", "[client][oauth]") {
  auto token_requests = 0;
  auto client =
      gsheetpp::BasicGoogleSheetsClient<gsheetpp::UserOAuth2Auth>{gsheetpp::UserOAuth2Auth{
                                                                      .client_id          = "client-id",
                                                                  ...
                                                                  [&](gsheetpp::detail::HttpRequest const& request) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> {
                                                                    ...
                                                                    return gsheetpp::detail::HttpResponse{
                                                                        .status_code = 200,
                                                                        .body        = R"({"access_token":"oauth-token","token_type":"Bearer","expires_in":3600,"refresh_token":"refresh-123"})",
                                                                    };
                                                                  }};
...
TEST_CASE("user oauth clears the authorization code after a successful exchange", "[client][oauth]") {
  auto client = gsheetpp::BasicGoogleSheetsClient<gsheetpp::UserOAuth2Auth>{gsheetpp::UserOAuth2Auth{
                                                                                .client_id          = "client-id",
                                                                                .client_secret      = "client-secret",
                                                                                .redirect_uri       = "http://127.0.0.1/callback",
                                                                                .authorization_code = "auth-code-123",
...
TEST_CASE("user oauth requires client_id", "[client][oauth]") {
  auto client = gsheetpp::BasicGoogleSheetsClient<gsheetpp::UserOAuth2Auth>{
      ...
      [](gsheetpp::detail::HttpRequest const&) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> { FAIL("missing client_id should fail before hitting the transport"); }};
                                                                  ...
                                                                  [&](gsheetpp::detail::HttpRequest const& request) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> {
                                                                    if (request.url == "https://oauth2.googleapis.com/token") {
                                                                      ...
                                                                      if (token_requests == 1) {
                                                                        return gsheetpp::detail::HttpResponse{
                                                                            .status_code = 200,
                                                                            .body        = R"({"access_token":"oauth-token-1","token_type":"Bearer","expires_in":3600,"refresh_token":"refresh-123"})",
                                                                        };
                                                                      }
      ...
      gsheetpp::BasicGoogleSheetsClient<gsheetpp::UserOAuth2Auth>{gsheetpp::UserOAuth2Auth{
                                                                  ...
                                                                  [&](gsheetpp::detail::HttpRequest const& request) -> std::expected<gsheetpp::detail::HttpResponse, gsheetpp::GoogleSheetsError> {
                                                                    ...
                                                                    if (request.headers.at(0) == "Authorization: Bearer oauth-token-1") {
                                                                      // 2 本のリクエストが同じ失効トークンで 401 を受ける状況を同期的に再現します。
